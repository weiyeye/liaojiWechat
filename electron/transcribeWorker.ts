...
async function run() {
    ...
    const emit = (msg: any): Promise<void> => {
        if (parentPort) {
    ...
    try {
        ...
        try {
            ...
            await emit({ type: 'error', error: 'Failed to load speech engine: ' + String(requireError) });
        ...
        if (isLanguageAllowed(result, allowedLanguages)) {
            ...
            await emit({ type: 'final', text: processedText })
